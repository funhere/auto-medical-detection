...
def summarize(tasks, models=('2d', '3d_lowres', '3d_fullres', '3d_cascade_fullres'),
              output_dir=join(net_training_out_dir, "summary_jsons"), folds=(0, 1, 2, 3, 4)):
    ...
    for model in models:
        for t in tasks:
            if not isdir(join(net_training_out_dir, model)):
                continue
            task_name = subfolders(join(net_training_out_dir, model), prefix="Task%02.0d" % t, join=False)
            if len(task_name) != 1:
                print("did not find unique output folder for net %s and task %s" % (model, t))
                continue
            task_name = task_name[0]
            out_dir_task = join(net_training_out_dir, model, task_name)
            ...
            for trainer in model_trainers:
                ...
                for v in validation_folders:
                    ...
                    if ok:
                        ...
                        classes_dice_mean(join(out_dir, "summary_allFolds__%s.json" % v))
                        classes_dice_mean(join(output_dir, "%s__%s__%s__%s.json" % (task_name, model, trainer, v)))
