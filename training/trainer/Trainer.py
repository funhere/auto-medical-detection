import matplotlib
from training.GenericTrainer import GenericTrainer
...
from preprocessing.dataset_generator import load_dataset, BatchGenerator3D, BatchGenerator2D, unpack_dataset
...
from datasets.data_augmentation.default_data_augmentation import get_default_aug, get_patch_size
...
class Trainer(GenericTrainer):
    def __init__(self, cf, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        super(Trainer, self).__init__(deterministic)
        self.cf = cf
        ...
        self.folder_with_preprocessing_data = None
        ...
        self.classes = self.do_dummy_2D_aug = self.use_mask_for_norm = self.store_largest_connected_component = \
    ...
    def update_fold(self, fold):
        """
        used in the ensemble of models from cross-validation
    ...
    def setup_DA_params(self):
        if self.threeD:
            self.data_aug_params = self.cf.aug_3D_kwargs
            if self.do_dummy_2D_aug:
                self.data_aug_params["dummy_2D"] = True
                self.write_to_log("Using dummy2d data augmentation")
                self.data_aug_params["elastic_deform_alpha"] = \
                    self.cf.aug_2D_kwargs["elastic_deform_alpha"]
                self.data_aug_params["elastic_deform_sigma"] = \
                    self.cf.aug_2D_kwargs["elastic_deform_sigma"]
                self.data_aug_params["rotation_x"] = self.cf.aug_2D_kwargs["rotation_x"]
        else:
            self.do_dummy_2D_aug = False
            if max(self.patch_size) / min(self.patch_size) > 1.5:
                self.cf.aug_2D_kwargs['rotation_x'] = (-15. / 360 * 2. * np.pi, 15. / 360 * 2. * np.pi)
            self.data_aug_params = self.cf.aug_2D_kwargs
    ...
    def initialize(self, training=True, force_load_plans=False):
        """
        :param training: 
            -False: prediction on test cases only
            -True: training 
        :return:
        """
        ...
        if training:
            ...
            if self.unpack_data:
                ...
                print("INFO: Not unpacking data! Training may be very slow. ")
        ...
        #assert isinstance(self.net, (DetectionNet, nn.DataParallel))
        self.was_initialized = True

    def init_net_optimizer_and_scheduler(self):
        """
        This is specific to the U-Net and must be adapted for other net architectures
        :return:
        """
        #self.write_to_log(self.net_num_pool_op_kernel_sizes)
        #self.write_to_log(self.net_conv_kernel_sizes)
        ...
        self.net = Generic_UNet(self.num_input_channels, self.base_num_features, self.num_classes, net_numpool,
                                    2, 2, conv_op, norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs,
                                    net_nonlin, net_nonlin_kwargs, False, False, lambda x: x, InitWeights_He(1e-2),
                                    self.net_num_pool_op_kernel_sizes, self.net_conv_kernel_sizes, False, True, True)
        self.optimizer = torch.optim.Adam(self.net.parameters(), self.initial_lr, weight_decay=self.weight_decay, amsgrad=True)
    ...
    def do_training(self):
        ...
        super(Trainer, self).do_training()
    ...
    def process_plans(self, plans):
        if self.stage is None:
            assert len(list(plans['plans_per_stage'].keys())) == 1, \
                "Please specify which stage of the cascade must be trained"
        ...
        self.store_largest_connected_component = plans['keep_only_largest_region']
    ...
    def load_dataset(self):
        self.dataset = load_dataset(self.folder_with_preprocessing_data)
    ...
    def get_basic_generators(self):
        ...
        if self.threeD:
            dl_tr = BatchGenerator3D(self.dataset_tr, self.basic_generator_patch_size, self.patch_size, self.batch_size,
                                 False, oversample_foreground_percent=self.oversample_foreground_percent,
                                 pad_mode="constant", pad_sides=self.pad_all_sides)
            dl_val = BatchGenerator3D(self.dataset_val, self.patch_size, self.patch_size, self.batch_size, False,
    ...
    def preprocess_patient(self, input_files):
        ...
        :return:
        """
        from preprocessing.preprocessing import GenericPreprocessor, Preprocessor2D
        if self.threeD:
            preprocessor = GenericPreprocessor(self.normalization_schemes, self.use_mask_for_norm,
                                               self.intensity_properties)
        else:
            preprocessor = Preprocessor2D(self.normalization_schemes, self.use_mask_for_norm,
                                             self.intensity_properties)

        d, s, properties = preprocessor.preprocess_test(input_files,
    ...
    def preprocess_predict_nifti(self, input_files, output_file=None, softmax_ouput_file=None):
        ...
        print("predicting...")
        pred = self.predict_preprocessing_return_softmax(d, True, 1, False, 1, (0, 1, 2), True, True, 2,
                                                             self.patch_size, True)  # TODO use da params for mirror
        print("resampling to original spacing and nifti export...")
        store_seg_from_softmax(pred, output_file, properties, 3, None, None, None, softmax_ouput_file,
                                             None)
        print("done")

    def predict_preprocessing_return_softmax(self, data, do_mirroring, num_repeats, use_train_mode, batch_size,
                                                 mirror_axes, tiled, tile_in_z, step, min_size, use_gaussian):

        assert isinstance(self.net, (DetectionNet, nn.DataParallel))
        return self.net.predict_3D(data, do_mirroring, num_repeats, use_train_mode, batch_size, mirror_axes,
    ...
    def validate(self, do_mirroring=True, use_train_mode=False, tiled=True, step=2, save_softmax=True,
        ...
        for k in self.dataset_val.keys():
            ...
            if override or (not isfile(join(output_folder, fname + ".nii.gz"))):
                ...
                softmax_pred = self.predict_preprocessing_return_softmax(data[:-1], do_mirroring, 1,
                ...
                results.append(export_pool.starmap_async(store_seg_from_softmax,
    ...
    def do_online_evaluation(self, output, target):
        with torch.no_grad():
    ...
    def finish_online_evaluation(self):
        ...
        self.write_to_log("Val glob dc per class:", str(global_dc_per_class))
    ...
    def store_checkpoint(self, fname, save_optimizer=True):
        super(Trainer, self).store_checkpoint(fname, save_optimizer)
        info = OrderedDict()
