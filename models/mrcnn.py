...
class Classifier(nn.Module):
    """
    Head net for classification and bounding box refinement. Performs RoiAlign, processes resulting features through a
...
class Mask(nn.Module):
    """
    Head net for proposal-based mask segmentation. Performs RoiAlign, some convolutions and applies sigmoid on the
...
def get_results(cf, img_shape, detections, detection_masks, box_results_list=None, return_masks=True):
    ...
    for ix in range(img_shape[0]):
        ...
        if 0 not in detections[ix].shape:
            ...
            # stages of training when the net weights are still a bit random.
            if cf.dim == 2:
            ...
            if return_masks:
                for i in range(masks.shape[0]):
                    # Convert neural net mask to full size mask.
...
class net(nn.Module):
    ...
    def train_forward(self, batch, is_validation=False):
        """
        train method (also used for validation monitoring). wrapper around forward pass of net. prepares input data
    ...
    def test_forward(self, batch, return_masks=True):
        """
        test method. wrapper around forward pass of net without usage of any ground truth information.
    ...
    def loss_samples_forward(self, batch_gt_class_ids, batch_gt_boxes, batch_gt_masks):
        ...
        """
        # sample rois for loss and get corresponding targets for all Mask R-CNN head net losses.
